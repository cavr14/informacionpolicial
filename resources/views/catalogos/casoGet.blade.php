...
<div class="row my-4">
    ...
    <div class="col-auto titlebar-commands">
        <a class="btn btn-primary" href="{{url('/catalogos/caso/agregar')}}">Agregar</a>
    </div>
</div>
            ...
            <tr>
                ...
                <td class="text-center">
                    <div class="action-buttons">
                        <a href="{{ url("/catalogos/caso/{$caso->idCaso}/detalles") }}" class="btn btn-warning btn-sm">Detalles</a>
                        <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modifyModal{{$caso->idCaso}}">Modificar</button>
                    </div>
                    <!-- Modal -->
                    <div class="modal fade" id="modifyModal{{$caso->idCaso}}" tabindex="-1" aria-labelledby="modifyModalLabel{{$caso->idCaso}}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modifyModalLabel{{$caso->idCaso}}">Confirmación de Modificación</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    ¿Estás seguro de que deseas modificar el caso de {{$caso->delito->nombre}}?
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                                    <a href="{{ url("/catalogos/caso/{$caso->idCaso}/modificar") }}" class="btn btn-primary">Continuar</a>
                                </div>
                            </div>
                        </div>
                    </div>
